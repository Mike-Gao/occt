...
typename std::enable_if<opencascade::is_integer<TheInteger>::value, Standard_Integer>::type IntegerHashCode (
  const TheInteger                                                theValue,
...
//! @tparam TheUnsignedInteger the type of the given value (it is "unsigned int", and must not be the same as
//! Standard_Size, because the overload of the HashCode function for Standard_Size type is already presented in
//! Standard_Size.hxx)
...
typename std::enable_if<!opencascade::std::is_same<Standard_Size, unsigned int>::value
