...
void Resource_Unicode::ConvertANSIToUnicode(const Standard_CString theFromStr, TCollection_ExtendedString& theToStr, const Standard_CString& theLocal)
{
  ...
  std::locale aLocation;
  try { aLocation = std::locale(aNameOfLocation.ToCString()); }
  catch (const Standard_CString)
  {
    TCollection_ExtendedString aCurrExtString(theFromStr,Standard_False);
    theToStr.AssignCat(aCurrExtString);
    return;
  }
  const Standard_Size aSizeOfIn = strlen(theFromStr);
  NCollection_Array1<Standard_WideChar> aStringUtf(1, aSizeOfIn + 1);
  const Standard_Character* anInPntr = 0;
  Standard_WideChar* anOutPntr = 0;
  mbstate_t aState;
  std::use_facet<std::codecvt<Standard_WideChar, Standard_Character, mbstate_t> >(aLocation).in
    (aState, theFromStr, theFromStr + aSizeOfIn, anInPntr, &aStringUtf.ChangeFirst(), &aStringUtf.ChangeLast(), anOutPntr);
  *anOutPntr = '\0';
  TCollection_ExtendedString aCurrExtString(&aStringUtf.First());
  theToStr.AssignCat(aCurrExtString);
}
                                                        ...
                                                        const Standard_Integer theMaxSize)
{
  ...
  const Standard_WideChar* anInPntr = 0;
  Standard_Character* anOutPntr = 0;
  mbstate_t aState;
  std::codecvt_base::result aRes = std::use_facet<std::codecvt<Standard_WideChar, Standard_Character, mbstate_t>>(aLocation).out
  (aState, theFromStr.ToWideString(), theFromStr.ToWideString() + theFromStr.Length(), anInPntr, theToStr, theToStr + theMaxSize + 1, anOutPntr);
  *anOutPntr = '\0';
  return aRes > 0;
}
