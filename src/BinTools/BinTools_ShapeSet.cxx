...
{
  if(theFormat >= BIN_TOOLS_VERSION_1 && theFormat <= THE_CURRENT_VERSION)
    myFormat = theFormat;
  else 
    myFormat = THE_CURRENT_VERSION;
}
                                ...
                                const Handle(Message_ProgressIndicator)& theProgress)const
{
  ...
  // write the copyright
  if (myFormat == BIN_TOOLS_VERSION_4)
    OS << "\n" << Version_4 << "\n";
  else if (myFormat == BIN_TOOLS_VERSION_3)
    OS << "\n" << Version_3 << "\n";
  else if (myFormat == BIN_TOOLS_VERSION_2)
    OS << "\n" << Version_2 << "\n";
  else
    OS << "\n" << Version_1 << "\n";
                               ...
                               const Handle(Message_ProgressIndicator)& theProgress)
{
  ...
  if (strcmp(vers, Version_4) == 0)
    SetFormat(BIN_TOOLS_VERSION_4);
  else if (strcmp(vers,Version_3) == 0) 
    SetFormat(BIN_TOOLS_VERSION_3);
  else  if (strcmp(vers,Version_2) == 0) 
    SetFormat(BIN_TOOLS_VERSION_2);
  else 
    SetFormat(BIN_TOOLS_VERSION_1);
  ...
  for (int i = 1; i <= nbShapes && aPSinner.More(); i++, aPSinner.Next()) {
    ...
    S.Free(aFree);
    S.Modified(aMod);
     if (myFormat >= BIN_TOOLS_VERSION_2)
       S.Checked(aChecked);
     else
    ...
    if (myFormat == BIN_TOOLS_VERSION_1)
      if(T == TopAbs_FACE) {
...
//       std::cout << "\nPOS = " << aPos << std::endl;
	...
	TopLoc_Location L;
	Standard_Boolean aNewF = (myFormat >= BIN_TOOLS_VERSION_3);
	do {
	  if(aNewF) {
