                                ...
                                Aspect_InteriorStyle& theStyle)
{
  ...
  else if (anArg == "solid")
  {
    theStyle = Aspect_IS_SOLID;
  ...
  else if (anArg == "solidwireframe"
        || anArg == "solid-wireframe"
        || anArg == "solid_wireframe"
        || anArg == "solidwf"
        || anArg == "solid-wf"
        || anArg == "solidedge"
        || anArg == "solid-edge"
        ...
        || anArg == "solid_edges")
  {
    theStyle = Aspect_IS_SOLID_WIREFRAME;
  }
  else if (anArg == "hatch")
  {
    theStyle = Aspect_IS_HATCH;
  }
  else if (anArg == "wireframe"
        || anArg == "hollow")
  {
...
struct ViewerTest_AspectsChangeSet
{
  ...
  Standard_Integer             ToSetInteriorQuad;
  ...
  bool Apply (const Handle(Prs3d_Drawer)& theDrawer)
  {
    ...
    if (ToSetInteriorQuad != 0)
    {
      if (ToSetInteriorQuad != -1
       || theDrawer->HasOwnShadingAspect())
      {
        toRecompute = theDrawer->SetupOwnShadingAspect (aDefDrawer) || toRecompute;
        theDrawer->ShadingAspect()->Aspect()->SetSkipFirstEdge (ToSetInteriorQuad == 1);
      }
    }
    if (ToSetEdgeWidth != 0)
    {
    ...
    if (ToSetTypeOfEdge != 0)
    {
       ...
       || theDrawer->HasOwnShadingAspect())
      {
        ...
        {
          if (ToSetTypeOfEdge == -1
           && theDrawer->ShadingAspect()->Aspect()->InteriorStyle() == Aspect_IS_SOLID_WIREFRAME)
          {
            theDrawer->ShadingAspect()->Aspect()->SetInteriorStyle (Aspect_IS_SOLID);
          }
          else if (ToSetTypeOfEdge == 1
                && theDrawer->ShadingAspect()->Aspect()->InteriorStyle() == Aspect_IS_SOLID)
          {
            theDrawer->ShadingAspect()->Aspect()->SetInteriorStyle (Aspect_IS_SOLID_WIREFRAME);
          }
        }
      }
    }
                                  ...
                                  const char**      theArgVec)
{
  ...
  {
    aChangeSet->ToSetInterior = 1;
    aChangeSet->InteriorStyle = Aspect_IS_SOLID_WIREFRAME;
  ...
  for (; anArgIter < theArgNb; ++anArgIter)
  {
    ...
    else if (anArg == "-setinteriorquad"
          || anArg == "-setquadinterior"
          || anArg == "-setquads"
          || anArg == "-quads"
          || anArg == "-skipfirstedge")
    {
      ...
      aChangeSet->ToSetInteriorQuad = isQuadMode ? 1 : -1;
    }
...
void ViewerTest::Commands(Draw_Interpretor& theCommands)
{
  ...
  theCommands.Add("vaspects",
      ...
      "\n\t\t:          [-unsetShadingModel]"
      "\n\t\t:          [-setInterior {solid|solidWireframe|hatch|hollow|hidenline|point}]"
      "\n\t\t:          [-unsetInterior] [-setHatch HatchStyle] [-interiorQuad {0|1}]"
