...
namespace
{
  ...
  static Standard_Integer GetArcNbPoints (const gp_Circ& theCircle,
                                          const Standard_Integer theNbPnts)
  {
                          ...
                          const Standard_Integer theNbPnts)
  {
    ...
    const Standard_Real aStep = (theU2 - theU1) / (theNbPnts - 1);
    ...
    for (Standard_Integer anIndex = 1; anIndex <= theNbPnts - 1; ++anIndex)
    {
      ...
      aV1.Normalize();
      thePolygon.SetPnt (aPntIdx - 1, aP1);
      ++aPntIdx;
      gp_Pnt aP2 (aP1.X() + aV1.X() * tan (aStep /2.0) * aRadius,
                  aP1.Y() + aV1.Y() * tan (aStep /2.0) * aRadius,
                  aP1.Z() + aV1.Z() * tan (aStep /2.0) * aRadius);
      thePolygon.SetPnt (aPntIdx - 1, aP2);
      ++aPntIdx;
      aCurU += aStep;
    }
    aP1 = ElCLib::CircleValue (theU2, theCircle.Position(), theCircle.Radius());
    thePolygon.SetPnt (theNbPnts * 2 - 2, aP1);
  }
}
